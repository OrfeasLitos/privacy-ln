\section{Lightning Protocol}
  ...
  Messages to and from \bob should be interpreted as \textsc{send} and
  \textsc{push} messages to and from $\anonfunc^{\mathrm{push}}$ with \bob as
  the sender and the receiver respectively. \bob is a ``logical'' (LN-only)
  address, therefore \alice need not know \bob's ``real'' address. This address
  is only known by $\anonfunc^{\mathrm{push}}$.
      ...
      \begin{algorithmic}[1]
        ...
        \Indent
          \ifelseieee{
            ...
            \emptyset$
            \State $\mathtt{unclaimedOfferedHTLCs} \gets \emptyset$
            \State $\mathtt{unclaimedReceivedHTLCs}, \mathtt{pendingGetPaid}
            \gets \emptyset$
          }{
            \State $\mathtt{channels}, \mathtt{pendingOpen},
            \mathtt{pendingPay}, \mathtt{pendingClose} \gets \emptyset$
        ...
        \Function{GetKeys}{}
          ...
          \label{alg:payprot:support:gen:fund}
          ...
          \label{alg:payprot:support:gen:rev}
    ...
    \environment}
      \begin{algorithmic}[1]
        ...
        \Indent
          ...
          channel before
          \label{alg:payprot:open:env:valid}
          ...
          1\right)$
          \label{alg:payprot:open:env:prf}
          ...
          \texttt{pendingOpen}
          \label{alg:payprot:open:env:pendingOpen}
          \State send (\textsc{openChannel}, $x,
          \mathtt{delay} + \tochain, ph_F, phb_{\mathrm{pay}},
          phb_{\mathrm{dpay}}, phb_{\mathrm{htlc}},$ $ph_{\mathrm{com}, 1},
          phb_{\mathrm{rev}}$, \textit{tid}) to \bob
        \EndIndent
      \end{algorithmic}
    \end{protocolbox}
    \caption{}
    \label{alg:payprot:open:env}
  \end{figure}
  ...
  \begin{figure}[H]
    \begin{protocolbox}{$\Pi_{\mathrm{LN}}$ -- \textsc{openChannel} from \bob}
      \begin{algorithmic}[1]
        ...
        \Indent
          ...
          \State $\mathrm{prand}_1 \gets \textsc{PRF}\left(\mathtt{seed},
          1\right)$
          \label{alg:payprot:open:peer:prf}
          \State $\left(ph_{\mathrm{com}, 1}, sh_{\mathrm{com}, 1}\right) \gets
          \textsc{KeyShareGen}\left(1^k; \mathrm{prand}_1\right)$
          \State associate keys with \textit{tid} and store in
          \texttt{pendingOpen}
          \label{alg:payprot:open:openChannel:pendingOpen}
    ...
    \end{protocolbox}
    \caption{}
    \label{alg:payprot:open:openChannel}
  \end{figure}
  ...
  \begin{figure}[H]
    ...
    \end{protocolbox}
    \caption{}
    \label{alg:payprot:open:acceptChannel}
  \end{figure}
  ...
  \begin{figure}[H]
    \begin{protocolbox}{$\Pi_{\mathrm{LN}}$ -- \textsc{fundingCreated}}
      \begin{algorithmic}[1]
        ...
        \Indent
          ...
          \label{alg:payprot:open:fundingCreated:mark}
    ...
    \end{protocolbox}
    \caption{}
    \label{alg:payprot:open:fundingCreated}
  \end{figure}
  ...
  \begin{figure}[H]
    \begin{protocolbox}{$\Pi_{\mathrm{LN}}$ -- \textsc{fundingSigned}}
      \begin{algorithmic}[1]
        ...
        \Indent
          ...
          \textsc{fundingLocked}''
          \label{alg:payprot:open:fundingSigned:mark}
    ...
    \end{protocolbox}
    \caption{}
    \label{alg:payprot:open:fundingSigned}
  \end{figure}
    ...
    \begin{protocolbox}{$\Pi_{\mathrm{LN}}$ -- \textsc{checkForNew}}
      \begin{algorithmic}[1]
        ...
        \Indent
          ...
          \label{alg:payprot:checkForNew:send}
        \EndIndent
      \end{algorithmic}
    \end{protocolbox}
    \caption{}
    \label{alg:payprot:checkForNew}
  \end{figure}
  ...
  \begin{figure}[H]
      ...
      \begin{algorithmic}[1]
        ...
        \Indent
          ...
          sent''} \Comment{i.e. marked with ``broadcast''}
            ...
            \State $\mathrm{prand}_2 \gets \textsc{PRF}\left(\mathtt{seed},
            2\right)$
            \label{alg:payprot:fundingLocked:prand}
            \State $\left(ph_{\mathrm{com}, 2}, sh_{\mathrm{com}, 2}\right) \gets
            \textsc{KeyShareGen}\left(1^k; \mathrm{prand}_2\right)$
          ...
          \label{alg:payprot:fundingLocked:report}
          \If{\texttt{channel} is not marked with ``\textsc{fundingLocked}
          sent''}
            \State replace ``broadcast'' mark in \texttt{channel} with
            ``\textsc{fundingLocked} sent''
    ...
    \end{protocolbox}
    \caption{}
    \label{alg:payprot:fundingLocked}
  \end{figure}
    ...
    \begin{protocolbox}{$\Pi_{\mathrm{LN}}$ -- poll}
      \begin{algorithmic}[1]
        ...
        \Indent
          \State send (\textsc{read}) to \ledger and assign reply to
          $\Sigma_{\alice}$
          \label{alg:payprot:poll:read}
          \State $\mathtt{toSubmit} \gets \emptyset$
          \label{alg:payprot:poll:afterread}
          \ForAll{$\tau \in \mathtt{unclaimedOfferedHTLCs}$}
            ...
            \EndIf
          \EndFor
          \State run loop of Fig.~\ref{alg:payprot:poll:closedch}
  ...
  \begin{figure}[H]
    ...
    poll}{commonbox}{normal}
      \begin{algorithmic}[1]
        ...
        \label{alg:payprot:poll:closedch:loop}
          \If{we do not have $sh_{\mathrm{rev}, n}$} \Comment{Honest closure}
            ...
            $\mathtt{remoteCom}_n$}
              ...
              \ElsIf{timelock has expired}
                \State add (sig, TX) to \texttt{toSubmit}
                \label{alg:payprot:poll:htlc:submit}
              \EndIf
            \EndFor
  ...
  \begin{figure}[H]
    \begin{protocolbox}{$\Pi_{\mathrm{LN}}$ -- pay}
      \begin{algorithmic}[1]
        ...
        \Indent
          ...
          \label{alg:payprot:pay:validpath}
          ...
          \label{alg:payprot:pay:validchannel}
          ...
          \label{alg:payprot:pay:sendinvoice}
        \EndIndent
        \Statex
    ...
    \end{protocolbox}
    \caption{}
    \label{alg:payprot:pay:outofband}
  \end{figure}
    ...
    \begin{protocolbox}{$\Pi_{\mathrm{LN}}$ -- invoice}
      \begin{algorithmic}[1]
        ...
        \Indent
          ...
          i + 1}$
          \label{alg:payprot:pay:invoice:cltv}
  ...
  \begin{figure}[H]
    \begin{protocolbox}{$\Pi_{\mathrm{LN}}$ -- \textsc{updateAddHtlc}}
      \begin{algorithmic}[1]
        ...
        \Indent
          \State run code of Fig.~\ref{alg:payprot:pay:updateAddHtlc:checks} --
    ...
    \end{protocolbox}
    \caption{}
    \label{alg:payprot:pay:updateAddHtlc}
  \end{figure}
  ...
  \begin{figure}[H]
    \begin{protocolbox}{$\Pi_{\mathrm{LN}}$ -- \textsc{updateAddHtlc} checks}
      \begin{algorithmic}[1]
        ...
        \Else \ \Comment{We are an intermediary}
          ...
          2\tochain$
          \label{alg:payprot:pay:updateAddHtlc:slack}
    ...
    \end{protocolbox}
    \caption{}
    \label{alg:payprot:pay:updateAddHtlc:checks}
  \end{figure}
  ...
  \begin{figure}[H]
      ...
      \begin{algorithmic}[1]
        ...
        \Indent
          ...
          where $h$ is the hash in the HTLC with number \texttt{HTLCNo}}
            ...
            Fig.~\ref{alg:payprot:close:unilateral})
            \State \Return
          \EndIf
          ...
          hash $h$ with number $\mathtt{HTLCNo}'$} \Comment{We are intermediary}
            ...
            \Comment{counterparty has gone on-chain}
              ...
              \texttt{HTLCTimeout}}
              \label{alg:payprot:pay:updateFulfillHtlc:submit}
            \Else \ \Comment{counterparty still off-chain}
              \State \Comment{Not having the HTLC irrevocably committed is
              impossible (Fig.~\ref{alg:payprot:pay:push},
              l.~\ref{alg:payprot:pay:pushadd:choose})}
    ...
    \end{protocolbox}
    \caption{}
    \label{alg:payprot:pay:updateFulfillHtlc}
  \end{figure}
    ...
    \begin{protocolbox}{$\Pi_{\mathrm{LN}}$ -- \textsc{commit}}
      \begin{algorithmic}[1]
        ...
        \Indent
          ...
          \textsc{revokeAndAck}''
          \label{alg:payprot:pay:commit:nomark}
          \State send (\textsc{read}) to \ledger and assign largest block
          number to $t$
          ...
          \label{alg:payprot:pay:commit:mark}
  ...
  \begin{figure}[H]
      ...
      \begin{algorithmic}[1]
        ...
        \Indent
          ...
          \mathtt{HTLCNo} - \mathtt{lastLocalSigned}$}
            \State close channel (as in Fig.~\ref{alg:payprot:close:unilateral})
            \State \Return
          \EndIf
          ...
          n+2\right)$
          \label{alg:payprot:pay:commitmentSigned:prf}
    ...
    \end{protocolbox}
    \caption{}
    \label{alg:payprot:pay:commitmentSigned}
  \end{figure}
    ...
    \begin{protocolbox}{$\Pi_{\mathrm{LN}}$ -- \textsc{revokeAndAck}}
      \begin{algorithmic}[1]
        ...
        \Indent
          ...
          \textsc{revokeAndAck}''
          \label{alg:payprot:pay:revokeAndAck:ensure}
          ...
          \label{alg:payprot:pay:raa:report}
          ...
          (Fig.~\ref{alg:payprot:close:coop}) has been initiated}
            \State continue execution at Fig.~\ref{alg:payprot:close:coop},
            l.~\ref{alg:payprot:close:coop:init:continue} or
            l.~\ref{alg:payprot:close:coop:shutdown:continue} respectively
          \EndIf
        \EndIndent
      \end{algorithmic}
    \end{protocolbox}
    \caption{}
    \label{alg:payprot:pay:revokeAndAck}
  \end{figure}
  ...
  \begin{figure}[H]
    \begin{protocolbox}{$\Pi_{\mathrm{LN}}$ -- \textsc{push}}
      \begin{algorithmic}[1]
        ...
        \environment:
        \label{alg:payprot:pushfulfill:start}
        \Indent
          \State ensure that there is a $\mathtt{channel} \in \mathtt{channels}$
          with ID \textit{pchid}
          ...
          \Else \ \Comment{counterparty gone on-chain}
            ...
            \label{alg:payprot:pushfulfill:submit}
          \EndIf
          \label{alg:payprot:pushfulfill:end}
        \EndIndent
        \Statex

        \State Upon receiving (\textsc{pushAdd}, \textit{pchid}) from
        \environment:
        \label{alg:payprot:pushadd:start}
        \Indent
          \State ensure that there is a $\mathtt{channel} \in \mathtt{channels}$
          with ID \textit{pchid}
          ...
          ``irrevocably committed'' $\mathtt{remoteCom}_n$ and
          $\mathtt{localCom}_n$
          \label{alg:payprot:pay:pushadd:choose}
          ...
          \label{alg:payprot:pushadd:end}
        \EndIndent
        \Statex

        \State Upon receiving (\textsc{fulfillOnChain}) from \environment:
        \label{alg:payprot:pay:foc:top}
        \Indent
          \State send (\textsc{read}) to \ledger and assign largest block
          number to $t$
          \label{alg:payprot:pay:foc:read}
          \State $\mathtt{toSubmit} \gets \emptyset$
          \label{alg:payprot:pay:foc:tosubmit}
          \ForAll{channels}
          ...
          \EndFor
          \State send (\textsc{submit}, \texttt{toSubmit}) to \ledger
          \label{alg:payprot:pay:foc:submit}
        \EndIndent
      \end{algorithmic}
    \end{protocolbox}
    \caption{}
    \label{alg:payprot:pay:push}
  \end{figure}
    ...
    \begin{protocolbox}{$\Pi_{\mathrm{LN}}$ -- close unilaterally}
      \begin{algorithmic}[1]
        ...
        \Indent
          ...
          \label{alg:payprot:close:unilateral:ensure}
          ...
          \State add \texttt{receipt}(\texttt{channel}) to
          \texttt{closedChannels}
          \label{alg:payprot:close:unilateral:report}
          \State remove \texttt{channel} from \texttt{channels}
          \label{alg:payprot:close:unilateral:remove}
          \State send (\textsc{submit}, $\left(\mathrm{sig},
          \mathtt{remoteSig}_n, \mathtt{localCom}_n\right), \mathrm{HTLCs}$) to
          \ledger
          \label{alg:payprot:close:unilateral:submit}
        \EndIndent
      \end{algorithmic}
    \end{protocolbox}
    \caption{}
    \label{alg:payprot:close:unilateral}
  \end{figure}
    ...
    \begin{protocolbox}{$\Pi_{\mathrm{LN}}$ -- close cooperatively}
      \begin{algorithmic}[1]
        ...
        \Indent
          ...
          \EndIf
          \label{alg:payprot:close:coop:init:continue}
        ...
        \Indent
          ...
          \If{\texttt{channel} is not marked ``coop closing''}
            ...
            $\mathtt{localCom}_n$}
              continue from here when there are none left
            \EndIf
            \label{alg:payprot:close:coop:shutdown:continue}
        ...
        \Indent
          \State ensure there is an open $\mathtt{channel} \in
          \mathtt{channels}$ with \bob with ID \textit{pchid}
          \label{alg:payprot:close:coop:signed:ensure:exists}
          \State ensure \texttt{channel} is marked as ``coop closing''
          \label{alg:payprot:close:coop:signed:ensure:marked}
          \State add \texttt{receipt}(\texttt{channel}) to
          \texttt{closedChannels}
          \label{alg:payprot:close:coop:report}
          \State remove \texttt{channel} from \texttt{channels}
          \State $Cl \gets$ TX \{input spends \texttt{channel} funding TX
          output, outputs pay $x, y$ to $pk_{\alice}, pk_{\bob}$ respectively,
          alphabetically ordered by some fixed encoding of the keys, where $x$
          ...
          \mathtt{True}$
          \label{alg:payprot:close:coop:signed:ensure:sig}
